mainApp.factory("CompanyMasterService", function($http, $location) {
	list = [];
	formData = [];
	progress = "";
	successMsg = "";
	...
	var endpoint = "/OCBS/rest/Company";
	...
	return {
		...
		stateList : function() {
			progress = "";
			var subpoint = "/OCBS/rest/State/searchState";
			$http({
				method : 'GET',
				url : subpoint,
			}).success(function(data, status, headers, config) {
		...
		cityList : function(stateId) {
			progress = "";
			var subpoint = "/OCBS/rest/City/searchCity";
			$http({
				method : 'GET',
				url : subpoint,
			}).success(function(data, status, headers, config) {
