mainApp.factory("CounterOfferService", function($http) {
	list = [];
	formData = [];
	progress = "";
	successMsg = "";
	...
	var endpoint = "/OCBS/rest/CounterOffer";
	...
	return {
		...
		/*
		 * listData : function(pageNo, pageSize) { progress = "Please Wait..";
		 * var subpoint = endpoint + "/search"; $http({ method : 'GET', url :
		 * subpoint, }).success(function(data, status, headers, config) { list =
		 * data; if (list.length != '' || list.length > 0) { progress = ""; }
		 * else { progress = "No Record Found."; } }).error(function(data,
		 * status, headers, config) { error = status + "Request Failed";
		...
		addData : function(counter, pageNo, pageSize) {
			var reload = this;
			successMsg = [];
			progress = "Please Wait..";
