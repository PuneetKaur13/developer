mainApp.factory("BannerManagementService", function($http) {
	list = [];
	formData = [];
	campaignList=[];
	progress = "";
	...
	var endpoint = "/OCBS/rest/BannerManagement";
	...
	return {
		...
		campaignList : function() {
			progress = "Please Wait..";
			var subpoint = "/OCBS/rest/CampaignManagement/search";
			$http({
				method : 'POST',
				url : subpoint,

			}).success(function(serverRes, status, headers, config) {
				campaignList = serverRes;
				if (campaignList.length != '' || campaignList.length > 0) {
					progress = "";
				} else {
					progress = "No Record Found..";
				}
