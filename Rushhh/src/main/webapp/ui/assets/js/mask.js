		...
		StringMask.prototype.process = function proccess(value) {
			...
			for (var i = steps.start; continueCondition(this.options); i = i + steps.inc) {
				...
				if (!token) {
					...
					formatted = concatChar(formatted, vc, this.options);
					valuePos = valuePos + steps.inc;
				} else if (!vc && this.options.usedefaults) {
					formatted = concatChar(formatted, this.options);
				} else {
					valid = false;
					break;
				}
...
(function() {
	...
	function validateBrPhoneNumber (ctrl, value) {
		var valid = ctrl.$isEmpty(value) || value.length === 10 || value.length === 10;
	...
	.directive('uiBrPhoneNumber',function() {
		 ...
		 */
		var phoneMask8D = new StringMask('000 0000-000'),
			phoneMask9D = new StringMask('00 00000-000');
